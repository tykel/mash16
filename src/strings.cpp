...
const char* const str_help[2] =
{
        ...
        "  --cpu-rec-1bblk-per-op limit recompiler basic blocks to 1 chip16 op each\n"
        "  --cpu-rec-no-fallback  do not fall back to interpreter for self-modifying code\n"
        "  --verbose              print debug information to standard output\n"
        "  --symbols=FILE         import debugging symbols from FILE (*.sym generated by as16)\n"
        "  --break@N,...          add breakpoint(s) at address(es) N,...\n"
        "  --break-all            break at every intruction\n"
        "  --watch@N,...          add memory watch points at address(es) N,...\n"
        "  --debugger=DEBUGGER    set debugger type to use (ui, stdout, both)\n"
        "  --rng-seed=SEED        set RNG seed to use, instead of time(NULL)\n"
