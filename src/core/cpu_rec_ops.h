...
#define EMIT_REX_RBI(reg, b, i, sz)                                            \
    do {                                                                       \
        int need = 0;                                                          \
...
// - cache the variable's address rather than its value -- this can't be used
// with flags.
//
...
#define HOSTREG_PTR(zzp)                                                       \
    cpu_rec_hostreg_var(state, reinterpret_cast<void *>(zzp), sizeof(void *),  \
                        CPU_VAR_ADDRESS_OF)
...
#define HOSTREG_STATE_VAR_RW_DIRTY(zzz, szz)                                   \
    cpu_rec_hostreg_var(state, &state->zzz, szz,                               \
                        CPU_VAR_READ | CPU_VAR_WRITE | CPU_VAR_DIRTY)
