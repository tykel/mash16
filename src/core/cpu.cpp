...
void cpu_init(cpu_state **state, uint8_t *mem, program_opts *opts)
{
    ...
    *state = (cpu_state *)mmap(0, total_alloc, PROT_READ | PROT_WRITE,
    ...
    (*state)->rec.bblk_map =
        reinterpret_cast<cpu_rec_bblk *>(calloc(65536, sizeof(cpu_rec_bblk)));
    (*state)->meta.old_pc = 0;
    (*state)->pc = 0;
    (*state)->m = mem;
#ifdef HAVE_BANK_SEL
    ...
    if (!((*state)->pal =
              reinterpret_cast<uint32_t *>(malloc(16 * sizeof(uint32_t))))) {
