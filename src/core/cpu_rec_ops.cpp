...
static void cpu_rec_op_jx_cx(cpu_state *state, int cx)
{
    ...
    int xbytes = cx ? (rexCallMov0 + 4 + rexCallAdd + 3 + rexCallMov1 + 5)
                    : (rexJmpMov + 5);
...
static void cpu_rec_op_mul_r3(cpu_state *state)
{
    ...
    cpu_rec_hostreg_convert_to_w_var(state, &state->r[i_z(state->i)], WORD,
                                     regXZReg);
}
...
static void cpu_rec_op_div_r3(cpu_state *state)
{
    ...
    int regSrcXReg =
        cpu_rec_hostreg_var(state, &state->r[i_yx(state->i) & 0x0f], WORD, 0);
